...
namespace GeneticSharp
{
    ...
    public sealed class GeneticAlgorithm : IGeneticAlgorithm
    {
        ...
        private IList<IChromosome> UnionToMutate(IList<IChromosome> offspring, IList<IChromosome> parents, int keepParents)
        {
            int minSize = parents.Count;
            if (keepParents > parents.Count) keepParents = parents.Count;
            int extraFromParents = minSize - offspring.Count - keepParents;
            if (extraFromParents < 0) extraFromParents = 0;
            int totalTake = keepParents + extraFromParents;

            //Os keep parents precisam ser clonados, para manter sua estrutura original
